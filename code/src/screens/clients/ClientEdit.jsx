...
export default function ClientEdit() {
  ...
  const [form, setForm] = useState({ name: row?.name || '', email: row?.email || '', phone: row?.phone || '', city: row?.city || '' })
  ...
  return (
    <form onSubmit={onSave} className="card" style={{maxWidth:800}}>
      <div className="section-title">تعديل بيانات العميل</div>
      <div className="grid cols-2">
        <div className="field"><label>الاسم</label><input className="input" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required /></div>
